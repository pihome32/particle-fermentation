...
boolean probe::_updateTemp() {  // read OneWire device temperature
  ...
  int16_t raw = (data[1] << 8) | data[0]; //Must be 16 bit int as the data from the sensor is 16 bits.  
  ...
  if ((double)raw *0.0625 <= 0 ) return false;  //return false if large negative value rturned.  Happens with a bad read on rare occasion. 
  // This is really a poor patch as it will block all reads if the sensor or connection is bad without any real error indication.  
  for (int i = 3; i > 0; i--) {
    _temperature[i] = _temperature[i - 1];
  }
  _temperature[0] = (double)raw * 0.0625;  // default is 12 bit resolution, 750 ms MAX conversion time
    #if DEBUG == true
