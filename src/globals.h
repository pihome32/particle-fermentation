...
#define ONE_WIRE_BUS D3  //Photon pin the one wire temperature probes are connected.
...
double lastOutput = 0;
double oldInput = 0;
double curFridgeTemp, curChamberTemp, curBoardTemp, curHeaterTemp, curRoomTemp;
...
const byte coolerFanPIN = A4;
const byte heaterFanPIN = A7;

const byte dacControlPIN = DAC1;
...
probe heater(&onewire), fridge(&onewire), chamber(&onewire), room(&onewire), board(&onewire) ;
